/**
 * Credit: https://github.com/jonschlinkert/detect-terminal
 */
import { execSync } from 'node:child_process'
import path from 'node:path'

export interface DetectOptions {
  /** When true, prefer outermost terminal (GUI app) over multiplexers like tmux/screen */
  preferOuter?: boolean
}
...
export const detectFromEnv = (options: DetectOptions = {}): string | null => {
  ...
  if (
    ...
    (typeof process.env.TERM_PROGRAM_VERSION !== 'undefined' &&
      /vscode/i.test(process.env.TERM_PROGRAM_VERSION))
  ) {
    return 'vscode'
  }
...
export const detectFromShell = (options: DetectOptions = {}): string | null => {
  ...
  try {
    ...
    if (!options.preferOuter) {
      if (/^tmux/.test(terminal)) return 'tmux'
      if (/^screen/.test(terminal)) return 'screen'
    }

    return terminal.replace(/[^a-z0-9]+/g, '_')
  } catch {
    return null
  }
}
...
export const detectFromProcessTitle = (): string | null => {
  ...
  if (/^foot/.test(processTitle)) return 'foot'
  if (/^rio/.test(processTitle)) return 'rio'
  if (/^ghostty/.test(processTitle)) return 'ghostty'
...
export const detectTerminal = (options: DetectOptions = {}): string => {
  let terminal = detectFromEnv(options)

  if (!terminal) {
    terminal = detectFromShell(options)
  }
